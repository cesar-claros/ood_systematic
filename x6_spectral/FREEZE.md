...
**Held-out ResNet18 semantics (pinned now, pre-freeze):** `scores_risk_resnet18/` contains only `_fix-config` exports, whose selection semantics differ from the dev targets. Before held-out scoring, regenerate the plain (hyperparameter-selected) ResNet18 aggregates on the HPC with `retrieve_scores.py --dataset $src --network bbresnet18 --scores-dir scores_risk_resnet18` (no `--fix-config`), so held-out targets share the dev semantics exactly. Fallback if the raw per-cell inputs are unavailable: score per (dropout, reward) slice on the fix-config files and require a sign majority across slices, reporting the semantics difference alongside.
