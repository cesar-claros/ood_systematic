"""L2 per-CSF binary with n_classes added alongside source (Cs=50, cv=5).

Feature set:
  - 8 strict Papyan NC metrics  (StandardScaler)
  - n_classes ordinal           (StandardScaler)
        cifar10 → 10, supercifar100 → 19, cifar100 → 100, tinyimagenet → 200
  - source one-hot              (drop_first → 3 binary features)
  - regime one-hot              (drop_first → 3 binary features)
  → 15 features after preprocessing.

Predictor:
  LogisticRegressionCV(Cs=50, cv=5, penalty='l2', solver='lbfgs') per CSF.

Tests whether enriching the feature set with the ordinal class-count signal
(on top of source identity) improves on the source-only headline at the
finer L2 grid.
...
Outputs:
  outputs/ablations/l2_nclasses_cs50/track1/<split>/
    preds.parquet
    coefficients.parquet
  outputs/figures/l2_nclasses_cs50_coefficients_heatmap_xarch.{pdf,png}
  outputs/figures/l2_nclasses_cs50_coefficients_heatmap_lopo.{pdf,png}
  outputs/20_ablation_l2_nclasses_cs50.md
"""
...
ABLATION_ROOT = "ablations/l2_nclasses_cs50"
...
def feature_names_after_preprocess(pipe: Pipeline) -> list[str]:
    ...
    nclass_names = list(pre.transformers_[1][2])
    ohe = pre.named_transformers_["cat"]
    cat_names = list(ohe.get_feature_names_out(["source", "regime"]))
    return nc_names + nclass_names + cat_names
...
def build_train(long_df, label_df, train_model_ids):
    ...
    if merged.empty:
        return None, None, None, []
    X = merged[NC_PRIMARY + ["n_classes", "source", "regime"]]
    Y = merged[csf_cols]
...
def build_test(long_df, test_model_ids):
    ...
    if feats.empty:
        return None, None
    X = feats[NC_PRIMARY + ["n_classes", "source", "regime"]]
...
def report(out_root, out_path):
    lines = ["# Ablation — L2 per-CSF binary with n_classes added (Cs=50, cv=5)\n\n"]
    lines.append("**Date:** 2026-05-04\n")
    lines.append("**Source:** `code/nc_csf_predictivity/ablations/l2_per_csf_nclasses_cs50.py`\n")
    lines.append(f"**Model:** `LogisticRegressionCV(Cs={L2_CS}, cv={L2_CV}, penalty='l2')` per CSF\n")
    lines.append("**Features:** 8 NC + n_classes (scaled) + source one-hot + regime one-hot = 15 features\n")
    lines.append("**Comparison:** vs L2 Cs=50 *without* n_classes (step 23 ablation) and vs L2 Cs=10 headline (step 12).\n\n")
    ...
    for split in SPLITS:
        ...
        # n_classes coefficients per CSF
        ncc = coefs[coefs["feature"] == "n_classes"][["csf", "coefficient"]]
        ncc_avg = ncc.groupby("csf")["coefficient"].mean().rename("n_classes_coef").reset_index()
        ncc_avg = ncc_avg.sort_values("n_classes_coef")
        lines.append(f"### n_classes coefficient per CSF (averaged across folds)\n\n")
        lines.append("Standardized-feature scale: positive ⇒ higher class count "
                     "increases competitive probability.\n\n")
        lines.append("```\n" + ncc_avg.round(3).to_string(index=False) + "\n```\n\n")

    # Set-regret comparison
    lines.append("## Set-regret comparison (imputed)\n\n")
    lines.append("Three-way comparison: L2 Cs=10 headline (step 12), "
                 "L2 Cs=50 without n_classes (step 23), and this run (L2 Cs=50 "
                 "with n_classes added).\n\n")
    for split in SPLITS:
        ...
        new_preds = pq.read_table(pp).to_pandas()

        # Cs=10 headline imputed
        ...
        # Cs=50 without n_classes (step 23) imputed — recompute on-the-fly
        cs50_no_nc_path = (out_root / "ablations" / "l2_cs50" / "track1" / split / "preds.parquet")
        cs50_no_nc_metrics = {}
        if cs50_no_nc_path.exists():
            p_old = pq.read_table(cs50_no_nc_path).to_pandas()
            for side in SIDES:
                ...
                for regime in ["near", "mid", "far"]:
                    ...
                    cs50_no_nc_metrics[(regime, side)] = mean

        rows = []
        for side in SIDES:
            eval_side = filter_to_side(eval_long, side)
            os_ = oracle_for_side(oracle_df, side)
            m = binary_per_row(new_preds, eval_side, os_)
            for regime in ["near", "mid", "far"]:
                sub = m[m["regime"] == regime]
                if sub.empty:
                    continue
                vals = sub["set_regret_raw_imputed"].astype(float).values
                new_mean, new_lo, new_hi = bootstrap_mean_ci(vals)
                ...
                cs50_no_nc_val = cs50_no_nc_metrics.get((regime, side), np.nan)
                rows.append({
                    "regime": regime, "side": side,
                    "Cs10_headline": round(cs10_val, 3) if not np.isnan(cs10_val) else None,
                    "Cs50_no_nclasses": round(cs50_no_nc_val, 3) if not np.isnan(cs50_no_nc_val) else None,
                    "Cs50_with_nclasses": round(new_mean, 3),
                    "ci_lo": round(new_lo, 3),
                    "ci_hi": round(new_hi, 3),
                    "empty_share": round(emp, 3),
                })
...
def main():
    ...
    for split in SPLITS:
        ...
        print(f"  l2_nclasses_cs50: {n:,} pred rows")
        cp = out_root / ABLATION_ROOT / "track1" / split / "coefficients.parquet"
        if cp.exists():
            coefs = pq.read_table(cp).to_pandas()
            ...
            plot_heatmap(
                coefs,
                title=("Per-CSF L2 logistic coefficients with n_classes added\n"
                       f"(Cs={L2_CS}, cv={L2_CV}, penalty='l2'; "
                       f"15 features = 8 NC + n_classes + source + regime; "
                       f"split = {label})"),
                out_base=fig_dir / f"l2_nclasses_cs50_coefficients_heatmap_{split}",
            )
            print(f"  wrote {fig_dir / f'l2_nclasses_cs50_coefficients_heatmap_{split}.pdf'}")

    report(out_root, out_root / "20_ablation_l2_nclasses_cs50.md")
    print(f"wrote {out_root / '20_ablation_l2_nclasses_cs50.md'}")
