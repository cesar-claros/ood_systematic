...
Given per-sample logits, labels, and an already-computed temperature T*,
produce a flat dict of scalar diagnostics that characterize the
miscalibration and geometry of the classifier head.
...
state CSFs read when ``temp_scaled=True``. T* itself is returned as a
scalar summary of miscalibration.

Designed to be merged into a per-configuration CSV keyed on
(architecture, study, dataset, dropout, run, reward), mirroring
...
def _softmax_diagnostics(logits: torch.Tensor, prefix: str) -> dict[str, float]:
    ...
    out: dict[str, float] = {}
    out.update(_scalar_stats(entropy,    f"{prefix}_entropy"))
    out.update(_scalar_stats(max_prob,   f"{prefix}_maxprob"))
    out.update(_scalar_stats(margin,     f"{prefix}_margin"))
    out.update(_scalar_stats(logit_norm, f"{prefix}_logitnorm"))
    out.update(_scalar_stats(kl_uniform, f"{prefix}_kl_uniform"))
    if k_top >= 3:
        out.update(_scalar_stats(topk[:, 1] - topk[:, 2], f"{prefix}_top23margin"))
    if k_top >= 4:
        out.update(_scalar_stats(topk[:, 2] - topk[:, 3], f"{prefix}_top34margin"))
    return out


def _logit_classmean_geometry(
...
def _logit_covariance_geometry(logits: torch.Tensor) -> dict[str, float]:
...
def compute_head_diagnostics(
    ...
    labels: torch.Tensor,
    temperature: float,
    logits_dist: torch.Tensor | None = None,
) -> dict[str, float]:
    """Return a flat dict of raw + scaled head-side diagnostics.
    ...
    labels : (N,) tensor
        Ground-truth labels. Currently unused (kept for future
    ...
    out.update(_softmax_diagnostics(logits,     prefix="raw"))
    out.update(_softmax_diagnostics(logits / T, prefix="scaled"))
    out.update(_logit_classmean_geometry(logits, labels))
    out.update(_logit_covariance_geometry(logits))
    if logits_dist is not None:
        mean_logits = logits_dist.mean(dim=2)
        out.update(_softmax_diagnostics(mean_logits,     prefix="raw_mcd"))
        out.update(_softmax_diagnostics(mean_logits / T, prefix="scaled_mcd"))
    return out
